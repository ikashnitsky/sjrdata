...
df |>
  ...
  geom_path(linewidth = 1, alpha = .5) +
  geom_label(
    aes(label = year |> str_sub(3, 4)),
    size = 3,
    label.padding = unit(.15, "line")
  ) +
  scale_color_viridis_d(NULL, begin = .2, end = .9, option = "H") +
  scale_x_continuous(position = "top") +
  labs(
    x = "2-year citation per document (JIF analogue)",
    y = "SJR index",
...
ggsave("/rout/sjr-nature-science.pdf", width = 8, height = 6)
...
df |>
  ...
  geom_text(
    data = . %>% filter(year == 2025) %>% distinct(title),
    aes(label = title),
    x = -.5,
    y = seq(3.2, 2.5, length.out = 6),
    hjust = 0,
    size = 4,
    fontface = 2
  ) +
  scale_x_continuous(limits = c(-0.6, 4), expand = FALSE)+
  ...
  labs(
    ...
    subtitle = "SCImago Journal Rank, 1999--2025",
...
df |>
  ...
  labs(
    ...
    y = "SJR index",
    title = "Selected demographic journals",
    subtitle = "SCImago Journal Rank, 1999-2025, via #rstats {sjrdata}",
    caption = "Ilya Kashnitsky @ikashnitsky.phd"
  )
...
df |>
    ...
    geom_col()+
    scale_fill_viridis_c(NULL, begin = .26, end = .92, option = "G") +
    ...
    labs(
        x = NULL,
        y = NULL,
        title = "Number of journals listed in Scopus",
        subtitle = "SCImago Journal Rank, 1999-2025, via #rstats {sjrdata}",
        caption = "Ilya Kashnitsky @ikashnitsky.phd"
    )

ggsave("/rout/sjr-total-n-journals.pdf", width = 6, height = 4.5)
